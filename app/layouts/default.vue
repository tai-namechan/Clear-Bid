...
<template>
  <div class="cb-app">
    <div v-if="route.path === '/login'" class="cb-main">
      <slot />
    </div>
    <div v-else-if="bootError" class="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
      <p class="text-sm text-red-600">{{ bootError }}</p>
      <NuxtLink class="text-xs font-semibold text-blue-600" to="/login">ログインへ</NuxtLink>
    </div>
    <div v-else-if="!authReady || !ready" class="flex h-full w-full items-center justify-center">
